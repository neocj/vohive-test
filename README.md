...
本修复版使用测试镜像标签 `ghcr.io/neocj/vohive-test:arm64-smsfix`，用于验证 QMI/WMS 接收短信时中文、日文、韩文、俄文、阿拉伯文和 emoji 等非 ASCII 正文不会丢失。升级只需要拉取新镜像并重建容器，不要删除宿主机持久化目录：

```sh
cd /opt/vohive
cp compose.yaml compose.yaml.bak
sed -i 's#ghcr.io/neocj/vohive-test:arm64#ghcr.io/neocj/vohive-test:arm64-smsfix#g' compose.yaml
docker compose pull
docker compose up -d --force-recreate
docker logs --tail 100 vohive
```
...
```sh
cd /opt/vohive
sed -i 's#ghcr.io/neocj/vohive-test:arm64-smsfix#ghcr.io/neocj/vohive-test:arm64#g' compose.yaml
docker compose pull
docker compose up -d --force-recreate
docker logs --tail 100 vohive
```
...
- GitHub Actions 手动触发后发布测试标签 `ghcr.io/neocj/vohive-test:arm64-smsfix` 和 `ghcr.io/neocj/vohive-test:arm64-smsfix-<commit-sha>`，并启用 provenance 与 SBOM；本修复验证完成前不覆盖稳定 `arm64` 标签。

本地构建 ARM64 镜像：

```sh
docker buildx build --platform linux/arm64 -t ghcr.io/neocj/vohive-test:arm64-smsfix --load .
```
